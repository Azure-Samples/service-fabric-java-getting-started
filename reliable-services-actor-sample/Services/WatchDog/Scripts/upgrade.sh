...
if [[ $params =~ "InstanceCount" ]]; then
    sfctl application upgrade --application-id WatchdogApplication --application-version ${version} --parameters "{\"InstanceCount\":\"-1\"}" --mode "Monitored"
else
    sfctl application upgrade --application-id WatchdogApplication --application-version ${version} --parameters "{}" --mode "Monitored"
fi
if [ $? -ne 0 ]; then
