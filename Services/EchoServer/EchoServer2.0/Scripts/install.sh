...
if [[ $params =~ "InstanceCount" ]]; then
    sfctl application upgrade --app-id fabric:/EchoServerApplication --app-version 2.0.0 --parameters "{\"InstanceCount\":\"-1\"}" --mode "Monitored"
else
    sfctl application upgrade --app-id fabric:/EchoServerApplication --app-version 2.0.0 --parameters "[]" --mode "Monitored"
fi
