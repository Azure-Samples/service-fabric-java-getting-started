#!/bin/bash
set -x
azure servicefabric application package copy CounterActorApplication fabric:ImageStore > /dev/null 2>&1
if [ $? -ne 0 ]; then
...
version=$(sed -e "s/xmlns/ignore/" CounterActorApplication/ApplicationManifest.xml | xmllint --xpath "string(//ApplicationManifest/@ApplicationTypeVersion)" -)
