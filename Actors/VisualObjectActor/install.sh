...
if [ $# -eq 0 ]
  then
    ...
    azuresfcli servicefabric application create --application-name fabric:/VisualObjectApplication --application-type-name VisualObjectsApplicationType --application-type-version 1.0.0
  elif [ $1 = 0 ]
  then
  ...
  then
    ...
    azuresfcli servicefabric application create --application-name fabric:/VisualObjectApplication  --application-type-name VisualObjectsApplicationType --application-type-version 1.0.0 --application-parameter "[{\"key\":\"InstanceCount\",\"value\":\"-1\"}]"
fi
